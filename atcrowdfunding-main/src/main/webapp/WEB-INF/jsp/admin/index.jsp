        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form id="queryForm" action="${PATH}/admin/index" method="post"
                          class="form-inline" role="form" style="float:left;">
                        <div class="form-group has-feedback">
                            <div class="input-group">
                                <div class="input-group-addon">查询条件</div>
                                <input class="form-control has-success" value="${param.condition}" name="condition"
                                       type="text" placeholder="请输入查询条件">
                            </div>
                        </div>
                        <button onclick="$('#queryForm').submit()" type="button" class="btn btn-warning"><i
                    ...
                    <sec:authorize access="hasRole('超级管理员')">
                    ...
                    <div class="table-responsive">
                        <table class="table  table-bordered">
                            ...
                            <c:forEach items="${page.list}" var="admin" varStatus="status">
                                <tr>
                                    <td>${status.count}</td>
                                    <td><input id="${admin.id}" class="select" type="checkbox"></td>
                                    <td>${admin.loginacct}</td>
                                    <td>${admin.username}</td>
                                    <td>${admin.email}</td>
                                    <td>
                                        <button type="button" class="btn btn-success btn-xs"
                                                onclick="window.location.href='${PATH}/admin/toAssignRole?id=${admin.id}'" ><i
                                ...
                                <td colspan="6" align="center">
                                    <ul class="pagination">
                                        <c:if test="${page.isFirstPage}">
                                            <li disabled="" class="disabled"><a href="">上一页</a></li>
                                        </c:if>
                                        <c:if test="${!page.isFirstPage}">
                                            <li>
                                                <a href="${PATH}/admin/index?condition=${param.condition}&pageNum=${page.prePage}">上一页</a>
                                            </li>
                                        </c:if>
                                        ...
                                        <c:forEach items="${page.navigatepageNums}" var="pagenums">
                                            <c:if test="${pagenums==page.pageNum}">
                                                <li class="active"><a
                                                        href="${PATH}/admin/index?condition=${param.condition}&pageNum=${pagenums}">${pagenums}<span
                                                        class="sr-only">(current)</span></a></li>
                                            </c:if>
                                            <c:if test="${pagenums!=page.pageNum}">
                                                <li>
                                                    <a href="${PATH}/admin/index?condition=${param.condition}&pageNum=${pagenums}">${pagenums}</a>
                                                </li>
                                            </c:if>
                                        </c:forEach>
                                        ...
                                        <c:if test="${!page.isLastPage}">
                                            <li>
                                                <a href="${PATH}/admin/index?condition=${param.condition}&pageNum=${page.nextPage}">下一页</a>
                                            </li>
                                        </c:if>
...
// 配置选中框
        ...
        $("#deleteBatchBtn").click(function () {
            var checkedBoxList=$(".select:checked");
